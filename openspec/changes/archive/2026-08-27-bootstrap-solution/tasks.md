...
- [x] 1.4 Set up a stable development code-signing identity for macOS and sign the spike binary. Verify: grant Accessibility once, rebuild, rerun, and confirm the grant persists without re-prompting. Ran under #15 and came back FAIL. **Re-tested 2026-09-02** (issue #31): 3 forced rebuild-then-run cycles of the still-ad-hoc-signed spike binary all passed on the first run. No signing identity was built — the system TCC database shows the grant belongs to Rider, the terminal parent every command in that environment inherits, not to the spike binary's own signature. Still open for a binary run outside Rider's ancestry (later reached, indirectly, via task 11/7.2's `launchctl bootstrap` test) or for the packaged app — deferred to change 12. See `design.md`'s *Task 1.4 re-test* section.
