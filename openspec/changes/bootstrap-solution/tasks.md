...
- [ ] 1.5a **(partially verified — see Windows spike results in `design.md`)** Spike SoundFlow: open a capture device at 48 kHz mono float32 and record 5 seconds to a raw file. Verify: sample count matches the duration, and it works on a device whose native rate is not 48 kHz (e.g. a 44.1 kHz input) — this is the check that decides whether the resampling stage stays deleted.
