# application-host Specification

## Purpose

The process shell that hosts Pisum Whisper: a tray-only / menu-bar-only application that owns the
dependency-injection composition root, the Avalonia run loop, and the lifetime of the native
resources (global keyboard hook, capture device) that every other capability borrows. It also fixes
the third-party stack those capabilities are built on, and requires that stack to be demonstrated on
both target platforms before anything depends on it.
