...
The last step of a dictation: getting the transcript out of this process and into the document the
user was already typing in. Neither target platform offers a way to insert text into another
application, so delivery is the system clipboard plus a synthetic paste keystroke — which means it
borrows something that belongs to the user and has to give it back. The capability is therefore one
indivisible sequence rather than a set of steps a caller may order: read the clipboard's previous
text, write the transcript, check that synthetic input can reach the focused window, paste, restore.
Its guards are invariants about each other — never restore after a failed paste, restore only what
is still ours, and never let a cancelled delivery leave the user's clipboard destroyed. It owns each
platform's paste keystroke and its pacing, the exclusion the transcript is written with so it does
not linger in clipboard history, and the copied-but-not-pasted outcome that preserves a result the
keystroke could not deliver.
