# startup-diagnostics Specification

## Purpose

Everything this application can normally say for itself arrives only after it has finished starting.
It is tray-only: no window, no taskbar entry, and in a release build no console — so its tray icon,
its settings window and its notifications are all surfaces that come into existence *after* the point
at which the failures here happen. A process that dies before its icon appears is therefore
indistinguishable, to the person who launched it, from one that never launched at all, and it is most
often launched from a login item where nobody is watching for it.

This capability is the answer to that. A failure that prevents startup is shown on a surface the
operating system owns rather than one this application builds, is recorded in the log before the
process goes, and ends the process with a non-zero exit code. It also covers the two conditions that
let the application start but leave it unable to do its job — a log that is not being written and a
binding that is not being observed — which are reported once a surface exists to report them on,
rather than at the moment they are discovered.

What such a message is allowed to say is bounded here as well. The one message composed from the
contents of the settings file is composed from a file holding the user's API keys in plaintext, and a
dialog drawn over whatever the user is presenting is a wider surface than the log file the existing
disclosure rules were written for.
