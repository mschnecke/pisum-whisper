...
The recording state machine: the capability that turns the global hotkey's two edges into a recording
and a recording into text at the cursor. It is what connects capture, encoding, transcription and
output to one another, and it is where every timing rule and concurrency guard in the product lives —
the minimum recording duration, the toggle debounce, the maximum-duration watchdog, the total
transcription budget, and the guarantee that only one dictation runs at a time.

It owns sequencing and failure handling rather than any of the steps it sequences: capture belongs to
`audio-capture`, encoding to `audio-encoding`, the upload to `gemini-transcription`, and the
clipboard and paste to `text-output`. What this capability adds is the order they run in, the state
it publishes while they run, and the rule that a failure is described to the user and never blocks
the next dictation.
