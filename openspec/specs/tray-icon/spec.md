...
The whole visible surface of a process that has no window, no taskbar button and no Dock icon. The
icon is how the user finds the application, judges what it is doing, and stops it, so it carries
three jobs at once: presence, state, and the two actions a tray-only application must expose to be
usable at all.

State is the part that is easy to get wrong. A dictation passes through three phases the user cannot
otherwise see — recording, uploading, done — and an icon with only two appearances has to lie about
one of them: showing "recording" through the upload tells a user in toggle mode to keep speaking
into a closed microphone, and showing "idle" tells them nothing is happening immediately before the
hotkey refuses a second dictation. So the capability requires three appearances, and requires them
to differ in shape rather than in colour, because macOS renders a menu bar icon from its silhouette
and discards its colours outright, and on Windows a hue-only difference conveys less than intended
to a colour-blind user.

Legibility is the other constraint: the user does not choose the background their tray icon is drawn
on, and an icon that disappears into it fails the only job it has — under either system appearance,
and across a change of appearance while the application is running, without the user configuring
anything. Beyond that the icon owns its tooltip, which names the active preset because that decides
what the model is told to do with the user's speech, and never the preset's system prompt; its menu,
which is Settings and Quit; and its own removal as the process exits, since an icon that outlives
its process claims the application is running when it is not and accumulates one entry per launch.
None of this may sit on the dictation's path — the state is reported from whichever thread the
dictation is running on, so presenting it must neither make that thread wait nor reorder what it
reported.
